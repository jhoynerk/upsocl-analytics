    ...
    f.inputs do
      f.has_many :countries_marks, heading: 'Países', allow_destroy: true, new_record: 'Añadir País' do |a|
        ...
        a.has_many :agencies_countries_marks, heading: 'Agencias', allow_destroy: true, new_record: 'Añadir Agencia', class: 'agencies_countries_panel' do |b|
          b.input :agency_id, label: 'Agencia', as: :select, collection: Agency.all.select(:name, :id ), input_html: { class: 'chosen-input select_agencia'}
          b.input :campaign_ids, label: 'Campaña', as: :select, collection: Campaign.all.select(:name, :id).map{|c| ["#{c.name} [#{c.id}]", c.id]} , input_html: { :multiple => true, class: 'chosen-input'}
        end
      end
    end
